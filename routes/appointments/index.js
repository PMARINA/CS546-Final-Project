...
router.post('/', async (req, res) => {
	...
	let success = false;
	// get date and time
	const startWashTime = moment(
		req.body.apptDate + ' ' + req.body.washerTime
	);
	const endWashTime = moment(
		req.body.apptDate + ' ' + req.body.washerTime
	).add(parseInt(req.body.washerCycle), 'minutes');
	// add washer cycle time
	// endWashTime.add(parseInt(req.body.washerCycle), 'minutes');
	// dryer cycle
	const startDryTime = moment(
		endWashTime.format('YYYY-MM-DD') + ' ' + req.body.dryerTime
	);
	const endDryTime = moment(
		startDryTime.format('YYYY-MM-DD') + ' ' + req.body.dryerTime
	).add(parseInt(req.body.dryerCycle), 'minutes');
	// get cycle id
	...
	for (let i = 0; i < appts.length; i++) {
		...
		if (
			(appts[i].machineId.toString() === req.body.whichWasher ||
				appts[i].machineId.toString() === req.body.whichDryer) &&
			appts[i].buildingId.toString() === req.body.apptBuilding
		) {
			const range = moment.range([
				moment(appts[i].startTimestamp),
				moment(appts[i].endTimestamp)
			]);
			if (apptRange.overlaps(range)) {
				success = false;
				res.render('apptOutcome', {
					navbar: req.navbar,
					success: success,
					message: 'Appointment times conflict. Please make a new appointment.'
				});
				return;
			}
			// check if machine is in maintenance
			for (let j = 0; j < maintenanceData.length; j++) {
				const maintenanceRange = moment.range([
					maintenanceData[j].startDate,
					maintenanceData[j].endDate
				]);
				if (apptRange.overlaps(maintenanceRange)) {
					...
					res.render('apptOutcome', {
						navbar: req.navbar,
						success: success,
						message: 'Machine is under maintenance during appointment time. Please make a new appointment.'
					});
					return;
				}
			}
	...
	// make appointments
	try {
		const newWashAppt = await AppointmentData.create(
			req.body.apptBuilding,
			req.userId,
			req.body.whichWasher,
			washerCycleId,
			startWashTime,
			endWashTime
		);
		const newDryAppt = await AppointmentData.create(
			req.body.apptBuilding,
			req.userId,
			req.body.whichDryer,
			dryerCycleId,
