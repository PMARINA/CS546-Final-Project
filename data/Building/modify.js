 ...
 * @param {String} buildingId the id of the building to modify
 ...
 */
async function validateAndCleanModifyBuilding(
    buildingId,
    name,
    location,
    washers,
  ...
  verify.general.verifyArg(buildingId, 'buildingId', 'validateAndCleanModifyBuilding', 'objectId');
  ...
  if (!await Building.exists({name})) {
    throw new Error('Refusing to modify a non-existant building');
  }
  ...
  return {id, name, location, washers, driers, accessGroups};
}
/**
 * Modify a building
...
async function modifyBuilding(
  ...
  ({buildingId, name, location, washers, driers, accessGroups} =
    await validateAndCleanModifyBuilding(
        buildingId,
        name,
        location,
        washers,
    ...
    let buildingToUpdate = await Building.findById(buildingId);

    buildingToUpdate.name = name;
    buildingToUpdate.location = location;
    buildingToUpdate.washers = washers;
    buildingToUpdate.driers = driers;
    buildingToUpdate.accessGroups = accessGroups;

    return await buildingToUpdate.save();

}

module.exports = {modifyBuilding};
