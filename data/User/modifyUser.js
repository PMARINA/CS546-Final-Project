...
async function validateUser(userId, argName) {
  if (typeof userId !== 'string') throw new Error(`The provided UserId (${argName}) was not a string`);
  let userObjectId;
  try {
    userObjectId = new ObjectId(userId);
  } catch (e) {
    throw new Error(`The provided UserId (${argName}) was not a valid ObjectId`);
  }
  if (userObjectId === null) throw new Error(`The provided UserId (${argName}) was null`);
  const userObject = await User.findById(userId).exec();
  if (userObject === null) throw new Error(`The provided UserId (${argName}) was not found in the database`);
}
 ...
 */
async function validateModification(userIdToModify, userIdAttemptingModification, params) {
  // Validate the user IDs
  await validateUser(userIdToModify, 'ID of User to Modify');
  await validateUser(userIdAttemptingModification, 'ID of User Attempting Modification');
  // Anyone can modify their own properties
  if (userIdToModify !== userIdAttemptingModification) {
    const roleOfUserToModify = (await User.findById(userIdToModify, {_id: 0, role: 1}).exec()).role;
    if (roleOfUserToModify === 'admin') throw new Error('Only an admin can modify their own user properties.');
    const roleOfModifier = (await User.findById(userIdAttemptingModification, {_id: 0, role: 1}).exec()).role;
    if (roleOfModifier !== 'admin') throw new Error('Only an admin can modify another user\'s params');
  } else {
    if (typeof params.role === 'string') {
      const requestedRole = params.role;
      const roleOfUserToModify = (await User.findById(userIdToModify, {_id: 0, role: 1}).exec()).role;
      if (requestedRole !== roleOfUserToModify) throw new Error('You are not allowed to change your own role');
  ...
  for (let [key, value] of Object.entries(params)) {
    ...
    if (validKeysForParams.has(key)) {
      if (typeof value === 'string') {
        value = value.trim();
        if (value === '') throw new Error(`Not allowed to set an empty (or spaces) for key: ${key}`);
        validatedParams[key] = value.trim();
      } else {
        throw new Error(`The object containing modifications contains a non-string param for key ${key}`);
      }
    } else {
      if (key === 'name') {
        if (typeof value !== 'object') throw new Error('The value for name must be an object.');
        ...
        for (let [keyInner, valueInner] of Object.entries(value)) {
          ...
          if (! validNameKeys.has(keyInner)) throw new Error(`name.${keyInner} is not a valid key`);
          if (typeof valueInner !== 'string') throw new Error(`The component of name (name.${keyInner}) must be a string`);
          valueInner = valueInner.trim();
          if (valueInner === '') throw new Error(`The component of name (name.${keyInner}) was blank or spaces`);
          if (validatedParams[key] === undefined) validatedParams[key] = {};
          validatedParams[key][keyInner] = valueInner;
        }
      } else {
        throw new Error(`It is not possible to modify the value in the user's document for key (${key})`);
      }
    }
  }
 ...
 */
async function checkUserAllowedToBeModified(userIdToModify, userAttemptingToModify) {
  ...
  if (roleOfObjectToModify === 'admin') throw new Error('Only the admin who uses this account modify its params');
}
 ...
 */
async function modifyUser(userIdToModify, userIdAttemptingModification, params) {
  await validateModification(userIdToModify, userIdAttemptingModification, params);
  await checkUserAllowedToBeModified(userIdToModify, userIdAttemptingModification);
  await User.updateOne({_id: new ObjectId(userIdToModify)}, Object.flatten(params));
}
