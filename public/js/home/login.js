  ...
  $(function () {
    ...
    async function formSubmit(emailField, pwdField, event) {
      ...
      if (!passwordValidation.valid) {
        ...
        passwordHelp.append(
          $('<p class="help is-danger">Your password is missing:</p>')
        );
        const ul = $('<ul class="ul"></ul>');
        if (passwordValidation.tooShort === true)
          ul.append(getElement("8+ characters in length"));
        if (passwordValidation.containsUpper === false)
          ul.append(getElement("Uppercase Character"));
        if (passwordValidation.containsLower === false)
          ul.append(getElement("Lowercase Character"));
        if (passwordValidation.containsNumber === false)
          ul.append(getElement("Digit"));
        if (passwordValidation.containsSymbol === false)
          ul.append(getElement("Symbol/Special Character"));
